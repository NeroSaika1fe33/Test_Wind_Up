...
//プレイヤーのパーツ管理
public class EnemyPartsContainer : EnemyComponent
{
    ...
    private void Awake()
    {
        string[] ramdomParts=PartsDataManager.Instance.GetRandomPartsArray();
        BodyPrefabName = PartsDataManager.Instance.Get_PartsID(ramdomParts[0]);
        MainspringPrefabName = PartsDataManager.Instance.Get_PartsID(ramdomParts[1]);
        FrontWheelPrefabName = PartsDataManager.Instance.Get_PartsID(ramdomParts[2]);
        BackWheelPrefabName = PartsDataManager.Instance.Get_PartsID(ramdomParts[2]);
