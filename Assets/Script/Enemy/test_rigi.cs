...

{
    ...
    private void Update()
    {
        ...
        Next_Corner.transform.position = NextCorner;
        n_rotate();
        //前進するコードをここに
        //------------------------------
    ...
    void n_rotate()
    {
        ...
        float Distance = Vector3.Distance(transform.position, Next_Corner.transform.position);
        ...
        if (NexCornerRight)
        {
            ...
            //次のコーナーが左側にあるなら
            if (Angle_Judgment.Left_RightJudgment(gameObject, Next_Corner.transform) == Direction.Left)
            {
                test += "コーナーが左:";
                LeftRotation = true;
            }
            //次のコーナーが右側にあるなら
            else if (Angle_Judgment.Left_RightJudgment(gameObject, Next_Corner.transform) == Direction.Right)
            {
                test += "コーナーが右:";
                if (Angle_Judgment.Front_Judgment(gameObject, Next_Corner.transform, new_angle_below))
                {
                    test += angle_below + "以下";
                    LeftRotation = true;
                }
                else if (!Angle_Judgment.Front_Judgment(gameObject, Next_Corner.transform, new_angle_above))
                {
                    test += angle_above +"以上";
                    RightRotation = true;
                }
        ...
        else
        {
            ...
            //Debug.DrawLine(gameObject.transform.position, gameObject.transform.position + transform.forward*5);
             Debug.Log(Angle_Judgment.Left_RightJudgment(gameObject, Next_Corner.transform));
            //次のコーナーが右側にあるなら
            if (Angle_Judgment.Left_RightJudgment(gameObject, Next_Corner.transform) ==　Direction.Right)
            {
                test += "コーナーが右:";
                RightRotation = true;
            }
            //次のコーナーが左側にあるなら
            else if (Angle_Judgment.Left_RightJudgment(gameObject, Next_Corner.transform) == Direction.Left)
            {
                test += "コーナーが左:";
                if (Angle_Judgment.Front_Judgment(gameObject, Next_Corner.transform, new_angle_below))
                {
